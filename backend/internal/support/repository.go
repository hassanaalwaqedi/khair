...
func (r *Repository) CreateTicket(ticket *models.SupportTicket) error {
	query := `INSERT INTO support_tickets (user_id, category, subject, status, priority)
			  VALUES ($1, $2, $3, $4, $5) RETURNING id, created_at, updated_at`
	
	return r.db.QueryRow(query, ticket.UserID, ticket.Category, ticket.Subject, ticket.Status, ticket.Priority).
		Scan(&ticket.ID, &ticket.CreatedAt, &ticket.UpdatedAt)
}

func (r *Repository) GetTicketByID(id uuid.UUID) (*models.SupportTicket, error) {
	query := `SELECT id, user_id, assigned_to, category, subject, status, priority, ai_summary, created_at, updated_at, first_human_response_at, resolved_at, closed_at
			  FROM support_tickets WHERE id = $1`
	
	t := &models.SupportTicket{}
	err := r.db.QueryRow(query, id).Scan(
		&t.ID, &t.UserID, &t.AssignedTo, &t.Category, &t.Subject, &t.Status, &t.Priority, &t.AISummary, &t.CreatedAt, &t.UpdatedAt, &t.FirstHumanResponseAt, &t.ResolvedAt, &t.ClosedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *Repository) GetAdminTickets(status string) ([]*models.SupportTicketWithDetails, error) {
	query := `SELECT t.id, t.user_id, t.assigned_to, t.category, t.subject, t.status, t.priority, t.ai_summary, t.created_at, t.updated_at, t.first_human_response_at, t.resolved_at, t.closed_at,
...
func (r *Repository) AssignTicket(ticketID, agentID uuid.UUID) (bool, error) {
	query := `UPDATE support_tickets SET assigned_to = $1, status = 'in_progress' 
...
func (r *Repository) CreateMessage(msg *models.SupportMessage) error {
	query := `INSERT INTO support_messages (ticket_id, sender_type, sender_user_id, body, message_type)
			  VALUES ($1, $2, $3, $4, $5) RETURNING id, created_at`
	
	return r.db.QueryRow(query, msg.TicketID, msg.SenderType, msg.SenderUserID, msg.Body, msg.MessageType).
		Scan(&msg.ID, &msg.CreatedAt)
}

func (r *Repository) GetTicketMessages(ticketID uuid.UUID, includeInternal bool) ([]*models.SupportMessage, error) {
	query := `SELECT m.id, m.ticket_id, m.sender_type, m.sender_user_id, m.body, m.message_type, m.created_at, m.read_at,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			&m.ID, &m.TicketID, &m.SenderType, &m.SenderUserID, &m.Body, &m.MessageType, &m.CreatedAt, &m.ReadAt, &m.SenderName,
