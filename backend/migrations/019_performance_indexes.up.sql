...
-- Note: CONCURRENTLY removed — golang-migrate runs inside transactions
...
CREATE INDEX IF NOT EXISTS idx_events_country_status ON events(country, status) WHERE status = 'published';
...
CREATE INDEX IF NOT EXISTS idx_badges_active ON organizer_badges(organizer_id, badge_type) WHERE is_active = true;

-- Payout reporting
CREATE INDEX IF NOT EXISTS idx_payouts_period ON payouts(organizer_id, period_start, period_end);
...
CREATE INDEX IF NOT EXISTS idx_events_upcoming ON events(start_date)
